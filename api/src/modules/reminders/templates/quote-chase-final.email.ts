...
export function quoteChaseFinalhHtml(data: {
  customerName: string;
  companyName: string;
  companyEmail: string;
  quoteNumber: string;
  ...
  return `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;color:#1a1a1a;">
      <div style="background:#dc2626;padding:24px 32px;border-radius:8px 8px 0 0;">
        ...
        <p style="color:rgba(255,255,255,0.8);margin:4px 0 0;font-size:13px;">Quote — Final Follow-Up</p>
      </div>
      <div style="border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px;padding:32px;">
        ...
        <p style="margin:0 0 20px;color:#555;line-height:1.6;">
          This is our final follow-up regarding quote <strong>${data.quoteNumber}</strong>
          for <strong>${gbp(data.totalPence)}</strong>.
          If you'd like to proceed, please click below${data.expiryDateStr ? ` before the quote expires on <strong>${data.expiryDateStr}</strong>` : ''}.
        </p>
        <p style="margin:0 0 24px;color:#555;line-height:1.6;">
          If you've decided not to go ahead, no worries — please just let us know and we won't
          bother you further.
        </p>
        ...
        <table style="width:100%;border-collapse:collapse;background:#f9fafb;border-radius:8px;margin-bottom:24px;">
          ...
          <tr style="border-top:1px solid #e5e7eb;">
            ...
            <td style="padding:10px 16px;font-size:15px;font-weight:700;color:#dc2626;">${gbp(data.totalPence)}</td>
          </tr>
          ${data.expiryDateStr ? `
          <tr style="border-top:1px solid #e5e7eb;">
            <td style="padding:10px 16px;font-size:13px;color:#888;">Expires</td>
            <td style="padding:10px 16px;font-size:13px;color:#dc2626;font-weight:600;">${data.expiryDateStr}</td>
          </tr>` : ''}
        </table>
