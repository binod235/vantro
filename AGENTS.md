...
# Codex priority instructions

Before making changes, audit existing code first.
Patch only gaps. Do not rebuild working modules.
Do not refactor unrelated files.
Follow existing Vantro patterns from Customers, Suppliers, Jobs, Quotes, Settings, and Billing.
Every DB query must filter by company_id.
Never trust company_id from frontend.
Money must be stored in pence.
Use Better Auth cookie sessions only. Never JWT. Never Clerk.
Run relevant TypeScript/build checks before reporting done.  
